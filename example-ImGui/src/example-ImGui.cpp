...
#pragma mark START TUTORIAL
    ...
    // To display them, you need a GUI. anu GUI could work with a minimal wrapper




    // OPTIONAL: Create an ImGUI
    ofxImGui::Gui gui;

    // original gui by https://github.com/ocornut/imgui wrapped for OF by https://github.com/jvcleave/ofxImGui
    // there are helper method to automatically construct a gui from controlfreak::Parameters
    // you don't need to instantiate an ofxImGui. becauses the addon has it's own.
    // But if you already have one in your app and want to use that instead of the addons one, you can
...
#pragma mark (ADVANCED) ERROR HANDLING
    ...
    void draw() {
        ...
        // this one line will draw the params on a gui
        gui.begin();
